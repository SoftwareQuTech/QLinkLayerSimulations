...
for name, times in dec_times.items():
    ...
    plt.plot(t_points, fid_points, label=name, linestyle='None', marker=markers[name], markersize=10)

plt.xlabel("Nr. of comm. rounds over 500 km")
plt.ylabel("Fidelity")
plt.xlim(0, 1)
plt.ylim(0.5, 1)
plt.xticks(t_points, range(len(t_points)))
plt.legend(loc="lower right")
# plt.savefig('fid_vs_comm.png')
plt.show()
